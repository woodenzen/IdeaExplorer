...
def main(zettel):
    ...
    for link in links:
        ...
        for file_name in file_names:
            ...
            file_tags[file_name] = [', '.join(tags)]

    # Create a DataFrame from the file_tags dictionary
    df = pd.DataFrame.from_dict(file_tags, orient='index', columns=['Tags'])

    # Add a column for the file names
    df['Note Name'] = df.index.str[33:-15] + '[[' + df.index.str[-15:-3] + ']]'
    ...
    # Reorder the columns
    df = df[['Note Name', 'Tags']]

        #Create HTML and Print html_table to a file
    html_table = df.to_html(index=False, escape=False, formatters=dict(Website=lambda x: '<a href="{}">{}</a>'.format(x, x)))
